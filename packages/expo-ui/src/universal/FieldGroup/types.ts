...
export interface FieldSectionProps extends UniversalBaseProps {
  ...
  /**
   * Background color applied to each row of the section. On Android and web
   * this fills the rounded-corner row container; on iOS SwiftUI's `Form`
   * handles row coloring natively and this prop is ignored.
   *
   * To adapt to dark mode on Android, pass a dynamic color such as
   * `Color.android.dynamic.surfaceContainerLow` from `expo-router`.
   *
   * @default '#FFFFFF'
   */
